...
function firstCard() {
	const cardHtml = results.map((results) => {
		return `
		<div class="card">
            <div class="cardIntro">
                <img class="cardImg" src=" ${results.headerImage.url}" alt="cardimage">
            <h2 class="cardTitle"> ${results.title}</h2>
			<p class="longtitle">${results.longTitle}</p>
	<p>fore more information <a href="${results.links.web}">here</a></p>
            </div>
         	<div class="buttonConts" >
                <button class="seemore">see more detals</button>
                <button class="remove">remove card</button>
            </div>
         </div>
		`
	})
    imageList.innerHTML = cardHtml
    
}

firstCard()
    
const buttonSeeMore = document.querySelectorAll(".seemore")
const buttonRemove = document.querySelectorAll('.remove')
const infodiv = document.querySelectorAll('.infodiv')
const moreInfoSection = document.createElement("div");
const cardintro = document.querySelectorAll(".cardIntro")
		
		
		
		buttonSeeMore.forEach((button, index) => {
